...
class StatisticsNotifier extends Notifier<StatisticsState> {
  ...
  IStatisticsRepository get _repository => ref.read(statisticsRepositoryProvider);

  Future<void> loadGlobalAnalytics() async {
    state = state.copyWith(isLoading: true, clearError: true);
    try {
      final response = await _repository.getAppAnalytics();
      if (response.success) {
      ...
      LoggerService.error('StatisticsNotifier.loadGlobalAnalytics', e);
    ...
    try {
      ...
      if (response.success && response.data != null) {
        ...
        state = state.copyWith(
          categoryStats: Map<String, dynamic>.from(currentMap).cast<String, dynamic>().map((k, v) => MapEntry(k, v)), 
          // Wait, casting is easier
          isLoading: false,
        );
        // Correct way to update map in immutable state
        final updatedStats = {...state.categoryStats, categoryId: response.data!};
